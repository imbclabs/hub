...
cheatas.Sailor.prototype =
{
	...
	setNCastData:function(data, key, seq, param)
	{
		...
		this.seq = this.getData(seq);
		this.type = "";
		this.key = this.getData(data.videoId);
		this.api = "";
		this.name = this.getData(data.title);
		this.image = this.getData(data.extraData.thumbUrl);
		this.imageCache = null;
		this.embed = this.getData("http://serviceapi.rmcnmv.naver.com/flash/outKeyPlayer.nhn?vid=" + this.key);
		this.description = this.getData(data.description);
		this.date = this.getData(new Date(data.produceDate));
		this.url = this.getData(data.destinationUrl);
		this.tags = cheatas.info.stringifyTags(cheatas.info.parseTags(this.name));
		this.isNCast = true;
	},
...
cheatas.Info.prototype =
{
	...
	loadSailors: function(api, sel, key, param)
	{
		...
		try
		{
			...
			if (api.indexOf("api.tvcast.naver.com") != -1)
			{
				this.loadSailorsFromNCast(api, sel, key, results);
				return;
			}
	...
	loadVideoInfoFromNCast: function(vid, delegate)
    {
		...
		cheatas.info["loadVideoInfoFromNCastComplete" + vid] = function(yn)
			{
				var iframePath = null;
				...
				if (yn == "N")
				{
					cheatas.lib.excuteDelegate(delegate, "loadError");
					return;
				}
				...
				iframePath = ajax.returnData.shareServicePlayerUrl;
				cheatas.lib.excuteDelegate(delegate, "loadComplete", [iframePath]);
			
				if(cheatas.info["loadVideoInfoFromNCastComplete" + vid] == this)
				{
					cheatas.info["loadVideoInfoFromNCastComplete" + vid] = null;
				}
			}

		ajax.request("http://mini.imbc.com/mininew/apiparser.aspx", param, "cheatas.info.loadVideoInfoFromNCastComplete" + vid, "POST", "text");
	},

	loadSailorsFromNCast: function (api, sel, key, results)
	{
		cheatas.debugger.log("Function",
			[
				"cheatas.Info.loadSailorsFromNCast(api, sel, key, results)",
				"api = " + api,
				"sel = " + sel,
				"key = " + key,
				"results = " + results
		...
		cheatas.info["loadSailorsComplete" + key + cheatas.config.KEY_RECEIVE] = function(yn)
			{
				...
				{
					sailor = new cheatas.Sailor();
					sailor.setNCastData(data[i], key, i, param);
					results.push(sailor);
				}
		...
		ajax.request("http://mini.imbc.com/mininew/apiparser.aspx", param, "cheatas.info.loadSailorsComplete" + key + cheatas.config.KEY_RECEIVE, "POST", "text");
	},

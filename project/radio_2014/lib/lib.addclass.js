	...
	resize : function(wid,hei)
	{
        ...
        switch(this.type){
			 ...
			 case jarvis.ImageLoader.FILL_CENTER:
                 bounceI=jarvis.lib.getEqualRatioFillRect(this.orgWidth,this.orgHeight,bounce.width,bounce.height);
				 break;
...
jarvis.GestureElement.prototype =
{
	...
	{
        var that=this;
		jarvis.lib.removeEventListener(this.view,"touchstart",function (e){ that.touchStart(e);});
		jarvis.lib.removeEventListener(this.view,"touchmove",function (e){  that.touchMove(e);});
		jarvis.lib.removeEventListener(this.view,"touchend",function (e){ that.touchEnd(e);});

		jarvis.lib.removeEventListener(this.view,"touchcancel",function (e){ that.touchEnd(e);});
		//jarvis.lib.removeEventListener(this.view,"webkitTransitionEnd",function (e){ that.touchEnd(e);});
		//jarvis.lib.removeEventListener(this.view,"msTransitionEnd",function (e){ that.touchEnd(e);});
		//jarvis.lib.removeEventListener(this.view,"oTransitionEnd",function (e){ that.touchEnd(e);});
		//jarvis.lib.removeEventListener(this.view,"transitionend",function (e){ that.touchEnd(e);});
	},
...
jarvis.DragElement= function(view,delegate,range) 
{
	...
	this.movePos=new Point();
    if(range==undefined){
	   range=jarvis.lib.getAbsoluteBounce(this.parent);
	   range.x=0;
	   range.y=0;
	}
	this.range=range;
    ...
    this.parentBounce=jarvis.lib.getAbsoluteBounce(this.parent);
    this.isDrag=false;
	var that=this;
    jarvis.lib.addEventListener(this.view,"mousedown",function(e) { that.dragStart(e);});	
	jarvis.lib.addEventListener(this.parent,"mousemove",function(e) { that.dragMove(e);});
	jarvis.lib.addEventListener(this.parent,"mouseup",function(e) { that.dragEnd(e);});
	jarvis.lib.addEventListener(this.parent,"mouseleave",function(e) { that.dragEnd(e);});
...
jarvis.DragElement.prototype =
{
	getPoint:function(e)
	{
        ...
        posX=posX-this.parentBounce.x-(this.viewW/2);
		posY=posY-this.parentBounce.y-(this.viewH/2);
		
       

		var maxX=this.range.x+this.range.width-(this.viewW);
		var maxY=this.range.y+this.range.height-(this.viewH);
        var minX=this.range.x;
		var minY=this.range.y;
	...
	dragMove:function(e)
	{
        ...
        this.view.style.left=jarvis.lib.getPx(this.movePos.x);
		this.view.style.top=jarvis.lib.getPx(this.movePos.y);
...
jarvis.MouseWheel.prototype =
{
	...
	giveFocus: function() {
		...
		//jarvis.lib.addEventListener(window,"mousewheel",function (e){ that.handleMousewheel(e);});
		//jarvis.lib.addEventListener(document,"mousewheel",function (e){ that.handleMousewheel(e);})
